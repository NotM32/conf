...
let
  repoConfig = {
    ...
    extraOptions = [
      ''sftp.command="/bin/sh -c 'ssh $(cat ${config.sops.secrets."backup_repo/connection".path}) -i ${
        config.sops.secrets."backup_repo/ssh_id".path
      } -o UserKnownHostsFile=${config.sops.secrets."backup_repo/known_hosts".path} -s sftp'" ''
    ];
  };
in
{
  ...
  config = {
    services.restic.backups = {
      ...
      srv = repoConfig // {
        ...
        extraBackupArgs = [
          "--option read-concurrency=10"
          "--tag automatic"
          "--tag ${self.shortRev or "dirtyflake"}"
        ];
      ...
      home = mkIf (config.backups.home.enable) (
        repoConfig
        // {
          paths = [
            "/home/m32/media"
      ...
      homeFull = mkIf (config.backups.home.enable) (
        repoConfig
        // {
          paths = [
            "/home/m32"
